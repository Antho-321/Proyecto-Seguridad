...
function DescargarComprobante() {
  ...
  filas_productos="";
  console.log(JSON.parse(localStorage.getItem('datos_carrito')));
...
function convertHtmlToData(htmlString) {
  let data = {
      cantidad_cliente: null,
      categoria: null, // assuming this needs to be extracted or set a static value
      cobertura: null,
      codigo_pastel: null, // assuming this needs to be extracted or set a static value
      dedicatoria: null,
      descripcion: null, // assuming this needs to be extracted or set a static value
      especificacion_adicional: null, // assuming this needs to be extracted or set a static value
      id_canasta: null, // assuming this needs to be extracted or set a static value
      img: null,
      masa: null,
      porciones: null, // assuming this needs to be extracted or set a static value
      precio: null,
      relleno: null,
      sabor: null,
      tamano: null // assuming this needs to be extracted or set a static value
  };

  // Extract the image URL
  let imgRegex = /<img src="(.*?)" alt=/;
  let imgMatch = htmlString.match(imgRegex);
  ...
  Object.keys(data).forEach(key => {
      if (key === 'img') return;
      let regex = new RegExp('<p class="col" name="' + key + '">\\n\\s*(.*?)</p>');
  ...
  // Handle specific fields with different patterns
  let precioRegex = /<p class="col" name="precio">\\n\\s*\$(.*?)<\/p>/;
  let precioMatch = htmlString.match(precioRegex);
  if (precioMatch && precioMatch[1]) {
      data.precio = precioMatch[1].trim();
  }

  let cantidadRegex = /<p class="col" name="cantidad">(.*?)<\/p>/;
  let cantidadMatch = htmlString.match(cantidadRegex);
  if (cantidadMatch && cantidadMatch[1]) {
      data.cantidad_cliente = cantidadMatch[1].trim();
  }

  // Set static or missing values if needed
  // Example: data.categoria = "Bautizos";
